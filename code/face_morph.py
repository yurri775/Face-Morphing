...
def morph_triangle(img1, img2, img, t1, t2, t, alpha) :
    ...
    # Skip degenerate or out-of-bounds rectangles
    if (r[2] == 0 or r[3] == 0 or r1[2] == 0 or r1[3] == 0 or r2[2] == 0 or r2[3] == 0 or
        r[0] < 0 or r[1] < 0 or r[0] + r[2] > w or r[1] + r[3] > h or
        r1[0] < 0 or r1[1] < 0 or r1[0] + r1[2] > w1 or r1[1] + r1[3] > h1 or
    ...
    if img1Rect.size == 0 or img2Rect.size == 0:
        return
    ...
    target_slice = img[r[1]:r[1]+r[3], r[0]:r[0]+r[2]]

    # Harmonize shapes to avoid broadcasting issues
    h = min(target_slice.shape[0], mask.shape[0], imgRect.shape[0])
    w = min(target_slice.shape[1], mask.shape[1], imgRect.shape[1])
    if h == 0 or w == 0:
        return

    target_slice = target_slice[:h, :w]
    imgRect = imgRect[:h, :w]
    mask = mask[:h, :w]

    blended = target_slice * (1 - mask) + imgRect * mask
    img[r[1]:r[1]+h, r[0]:r[0]+w] = blended
